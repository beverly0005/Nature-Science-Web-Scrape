...
for year in years:
    ...
    if os.path.isfile(datapath + FILENAME_AFFIL + str(year) + '.csv') is True:
        oldfile_affil = pd.read_csv(datapath + FILENAME_AFFIL + str(year) + '.csv', encoding='ISO-8859–1')
    ...
    with open(datapath + FILENAME_AUTHOR + str(year) + '.csv', mode, newline='') as csv_author, \
            open(datapath + FILENAME_AFFIL + str(year) + '.csv', mode, newline='') as csv_affil:  # create a csv file
